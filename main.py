...
class MainApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ATLAS - Automated Test Learning & Analytics System v1.10.0")
 	 ...
 	 # <-- 为新功能创建 Frame
        ...
        # 3. 将容器加入到标签页中
        # ==========================================
        # self.notebook.add(self.tab_data_preprocessing, text="解压 / 改名（可选）")
        self.notebook.add(self.tab_automatic_yield, text="数据清洗与良率汇总")
        # self.notebook.add(self.tab_cleaning, text="数据汇总清理 (Cleaning)")
        # self.notebook.add(self.tab_yield_summary, text="良率与BIN汇总报表 (Yield Summary)")
        # self.notebook.add(self.tab_merge_bin, text="数据提取整合（可选导出）")
        ...
        # 调用新模块的标准接口
        # try: wafer_yield_summary.create_ui(self.tab_yield_summary)
        # except Exception as e: print("加载 wafer_yield_summary 失败:", e)
